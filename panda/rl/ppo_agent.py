...
class PPOAgent(BaseAgent):
    def __init__(self, config, ob_space, ac_space, actor, critic):
        ...
        # build up networks
        self._actor = actor(config, ob_space, ac_space, config.tanh_policy) # actor model (MLP)
        ...
        self._buffer = ReplayBuffer(buffer_keys, config.buffer_size, sampler.sample_func)
    ...
    def state_dict(self):
        return {
            'actor_state_dict': self._actor.state_dict(),  # state_dict contains info about model params (learnable tensors: weights&biases)
            'critic_state_dict': self._critic.state_dict(),
            'actor_optim_state_dict': self._actor_optim.state_dict(),  # state_dict contains info about optim state and hyperparams
    ...
    def load_state_dict(self, ckpt):
        ...
        optimizer_cuda(self._actor_optim, self._config.device) # required when loading optim state from checkpoint
    ...
    def _update_network(self, transitions):
        ...
        o = transitions['ob'] #  dict {'robot-state': array(), 'object-state': array()} coming from Sampler
        o = self.normalize(o)
        ...
        a_z = _to_tensor(transitions['ac_before_activation']) # is OrderedDict([('default', tensor()]; tensor shape is (batch_size, 8)
        ret = _to_tensor(transitions['ret']).reshape(bs, 1)
        adv = _to_tensor(transitions['adv']).reshape(bs, 1)

        log_pi, ent = self._actor.act_log(o, a_z) # log_pi is tensor of shape (batch_size, 1)
        ...
        surr1 = ratio * adv
        surr2 = torch.clamp(ratio, 1.0 - self._config.clip_param, 1.0 + self._config.clip_param) * adv
