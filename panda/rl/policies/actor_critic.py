...
from rl.policies.distributions import FixedNormal, MixedDistribution
...
class Actor(nn.Module):
    def __init__(self, config, ob_space, ac_space, tanh_policy):
        super().__init__()
        self._config = config
        self._activation_fn = getattr(F, config.rl_activation) # elu, relu, tanh
    ...
    def act(self, ob, is_train=True, return_log_prob=False):
        ...
        for k in self._ac_space.keys():
            dists[k] = FixedNormal(means[k], stds[k]) # Normal dist with double args Normal(loc: , scale: )
    ...
    def act_log(self, ob, activations=None):
        ...
        log_probs_ = torch.cat(list(log_probs.values()), -1).sum(-1, keepdim=True) # log_probs_ is a tensor of shape (batch_size, 1)
