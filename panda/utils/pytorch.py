...
def obs2tensor(obs, device):
    ...
    return OrderedDict([(k, torch.tensor(np.stack(v), dtype=torch.float32).to(device)) for k, v in obs.items()])


# transfer a numpy array/list into a tensor
def to_tensor(x, device):
    if isinstance(x, dict):
        return OrderedDict([(k, torch.tensor(v, dtype=torch.float32).to(device)) for k, v in x.items()])
